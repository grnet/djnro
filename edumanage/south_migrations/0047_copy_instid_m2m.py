...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for server in orm.InstServer.objects.all():
            for server1 in server.instid_m2m.all():
                server.instid.add(server1)
            server.save()

    def backwards(self, orm):
        "Write your backwards methods here."
        for server in orm.InstServer.objects.all():
            for server1 in server.instid.all():
                server.instid_m2m.add(server1)
            server.save()

...
This change has a side effect on the command `./manage.py fetch_kml`. The default maximum item size for `PyMemcacheCache` is 1 Megabyte, but the kml file cached by DjNRO can be greater than 1 Megabyte. This can cause the command to fail due to being unable to cache the entire file.

To fix this, find the memcached configuration file (usually called `memcached.conf`), and change or add the option that controls the maximum item size. If no such file exists, one can be created, which will overwrite memcached's default settings.
```

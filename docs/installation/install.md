...
In order to start using DjNRO you need to create a Realm record for your NRO along with one or more contacts linked to it. You can log into the Django admin interface `https://<hostname>/admin` as superuser and add a Realm (remember to set REALM_COUNTRIES in local_settings.py).
